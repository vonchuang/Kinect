...
const	int	HAND_UP = 150;		//手掌上方可能存在指尖的区域，单位毫米
...
//					CameraSpacePoint	cameraShoulderRight = jointArray[JointType_ShoulderRight].Position;
	...
	//				myMapper->MapCameraPointToDepthSpace(cameraShoulderRight, &depthHandRight);
				...
				//Left Hand state(open -> press mouse , close -> release mouse)							
				HandState leftHandState = HandState::HandState_Unknown;
				hResult = bodyArray[i]->get_HandLeftState(&leftHandState);
				if (SUCCEEDED(hResult)) {
					ColorSpacePoint colorSpacePoint = { 0 };
					hResult = myMapper->MapCameraPointToColorSpace(jointArray[JointType::JointType_HandLeft].Position, &colorSpacePoint);
					if (SUCCEEDED(hResult)) {
